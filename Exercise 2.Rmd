...
par(mfrow=c(1,2))
ggplot(arr,aes(x=DayOfWeek, y = arr_delay + scale_x_date(labels=date_format("%a")))) + geom_line() + geom_point()+ labs(title = "Arrival delays of flights per day in May - Aug 2008", x = "Day of week",y="Mean of arrival delays")
...
```{r Boosting, warning=FALSE, paged.print=FALSE}
library(gbm)
#mode("X") = "numeric"
set.seed(12345)
...
```{r RF, warning=FALSE, paged.print=FALSE}
...
```{r NB, warning=FALSE, paged.print=FALSE}
library(foreach)
library(doParallel)
set.seed(12345)
registerDoParallel()
