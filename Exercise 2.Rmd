...
cls = c(1,4)
perf = unique(oper[,cls])

ggplot(perf, aes(x = UniqueCarrier , y = performance)) + geom_bar(stat = "identity", fill = "#0000CC") + labs(title = "Operational Delays of Carriers in 2008", x = "Carriers", y="Operational Delays")
```
...
par(mfrow=c(1,2))
ggplot(delays,aes(x=DayOfWeek, y = arr_delay)) + geom_line() + geom_point()+ labs(title = "Arrival delays of flights per day in May - Aug 2008", x = "Day of week",y="Mean of arrival delays") 
ggplot(delays,aes(x=DayOfWeek, y = dep_delay)) + geom_line() + geom_point()+ labs(title = "Departure delays of flights per day in May - Aug 2008", x = "Day of week",y="Mean of departure delays")
```
...
We will now create the corpus for all authors.
...
Creating test data corpus and ignoring words that arent present in the train data
...
Using the TF-IDF scores, we will build the classification model
...
dim(tfidf_train_df) #2500 660
tfidf_train_df[ , apply(tfidf_train_df, 2, var) != 0]
dim(tfidf_train_df) 
pc_author = prcomp(tfidf_train_df)

pc_author_test <- predict(pc_author, newdata = tfidf_test_df)
pc_author_test <- as.data.frame(X_test)

pve = summary(pc_author)$importance[3,]
plot(pve)
```

Creating Principal Components for training and testing data
...
pc_author = prcomp(tfidf_train_df)
pc_author_test <- predict(pc_author, newdata = tfidf_test_df)
plot(summary(pc_author)$importance[3,])
```
